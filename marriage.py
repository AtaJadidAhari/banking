        ...
        if people[i].gender == 0 and not people[i].married and people[i].alive:  # bachelors
            if random.random() < 0.1:  # TODO fix probabilty
                ...
                for j in range(len(people)):
                    if province_neighbor(people[i], people[j]) and economic_neighbor(people[i], people[j]) and i != j:
                ...
                for j in range(len(neighbors)):
                    if b - gammaBar <= neighbors[j].age <= b + gammaBar and \
                            religion_neighbor(people[i], neighbors[j]) and \
                            education_neighbor(people[i], neighbors[j]) and i != j:
                        social_network.append(people[j])
                potential_wives = []

                if len(social_network) == 0:
                    continue
                social_pressure = math.exp(beta * (probability_of_marriage(social_network) - alpha)) / (
                        1 + math.exp(beta * (probability_of_marriage(social_network) - alpha)))

                set_parameters(social_pressure)
                for j in range(len(social_network)):
                    if social_network[j].gender == 1 and \
                            not social_network[j].married and \
                            social_network[j].alive and religion_neighbor(people[i], social_network[j], 1) and \
